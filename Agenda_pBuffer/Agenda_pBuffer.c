...
void initBuffer(void **pBufferPtr) {
  *pBufferPtr = calloc(1, DATA_START_AREA); //  espaco para as variaveis de
                                            //  controle e o buffer temporario
...
void addPerson(void **pBufferPtr) {
  ...
  //  Layout Calculo:               TAMANHO NOME(+ \0) + TAMNHO EMAIL(+ \0) +
  //  TAMNHO INTEIRO
  *TOTAL_PERSON_SIZE(pBuffer) = (strlen(TEMP_NAME(pBuffer)) + 1) +
                                (strlen(TEMP_EMAIL(pBuffer)) + 1) + sizeof(int);
  ...
  *pBufferPtr = newBuffer;
  ...
  Depois eu atualizo o USED_DATA(para conter ja a quantidade do nome), para o
  email, depois faco a mesma coisa com email, para eu armazenar a idade, e pulo
  para o final da do inteiro
  */

  //            Destino                                origem numero de bytes
  memcpy((char *)pBuffer + *USED_DATA(pBuffer), TEMP_NAME(pBuffer),
         strlen(TEMP_NAME(pBuffer)) + 1);
  //  USED_DATA = USED_DATA + tamanho do nome + \0
  *USED_DATA(pBuffer) += strlen(TEMP_NAME(pBuffer)) + 1;

  //            Destino                                origem numero de bytes
  memcpy((char *)pBuffer + *USED_DATA(pBuffer), TEMP_EMAIL(pBuffer),
         strlen(TEMP_EMAIL(pBuffer)) + 1);
...
void removePerson(void **pBufferPtr) {
  ...
  for (*I(pBuffer) = 0; *I(pBuffer) < *COUNT(pBuffer); (*I(pBuffer))++) {
    ...
    if (pMoveTo == NULL && strcmp(TEMP_NAME(pBuffer), pReader) == 0) {
      ...
      // O tamanho da pessoa(para o realloc)  = Tamanho nome     + Tamanho do
      // E-mail                         +  Tamanho da Idade(inteiro)
      *TOTAL_PERSON_SIZE(pBuffer) =
          (strlen(pReader) + 1) +
...
void searchPerson(void *pBuffer) {
  ...
  // Se percorer toda a agenda e nao enocontrar printa que nao foi encontrado na
  // agenda
  printf("ERROR: %s not found\n", TEMP_NAME(pBuffer));
}
